...
function cli_help {
    ...
    usage+=" [-b|--blockchain-id=\${AVAX_BLOCKCHAIN_ID}]" ;
...
function rpc_method {
    printf "ipcs.unpublishBlockchain" ;
}

function rpc_params {
    printf '{' ;
    printf '"blockchainID":"%s"' "$AVAX_BLOCKCHAIN_ID" ;
    printf '}' ;
}

###############################################################################

cli "$@" && rpc_post "$AVAX_NODE/ext/ipcs" "$(rpc_data)" ;
