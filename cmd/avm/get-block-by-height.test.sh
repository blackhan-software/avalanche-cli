...
function cmd {
    printf "./avalanche-cli.sh avm get-block-by-height" ;
}
...
function check {
    ...
    expect_d+='"id":1,' ;
    expect_d+='"method":"avm.getBlockByHeight",' ;
    expect_d+='"params":{' ;
...
function test_avm__get_block_by_height_1a {
    check "$(AVAX_ID_RPC=1 $(cmd) -H HEIGHT -e hex)" ;
}

function test_avm__get_block_by_height_1b {
    check "$(AVAX_ID_RPC=1 AVAX_HEIGHT=HEIGHT $(cmd) -e hex)" ;
}

function test_avm__get_block_by_height_1c {
