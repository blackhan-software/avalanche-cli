...
function cli_help {
    ...
    usage+=" [-b|--block-id=\${AVAX_BLOCK_ID}]" ;
...
function cli_options {
    local -a options ;
    options+=( "-b" "--block-id=" ) ;
    options+=( "-e" "--encoding=" "--encoding=hex" "--encoding=json" ) ;
    options+=( "-N" "--node=" ) ;
...
function rpc_params {
    printf '{' ;
    printf '"blockID":"%s",' "$AVAX_BLOCK_ID" ;
...
cli "$@" && rpc_post "$AVAX_NODE/ext/X" "$(rpc_data)" ;
