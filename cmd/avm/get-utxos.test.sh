...
function test_avm__get_utxos_1a {
    check "$(AVAX_ID_RPC=1 $(cmd) -@X1 -@X2 -@X3 -l 1024 \
        --start-index-address=SI_X0 --start-index-utxo=SI_UTXO -s P)" ;
}
...
function test_avm__get_utxos_1d {
    check "$(AVAX_ID_RPC=1 AVAX_START_INDEX_ADDRESS=SI_X0 \
        $(cmd) -@X1 -@X2 -@X3 --limit=1024 --start-index-utxo=SI_UTXO -s P)" ;
}

function test_avm__get_utxos_1e {
    check "$(AVAX_ID_RPC=1 AVAX_START_INDEX_UTXO=SI_UTXO \
        $(cmd) -@X1 -@X2 -@X3 --limit=1024 --start-index-address=SI_X0 -s P)" ;
}

function test_avm__get_utxos_1f {
    check "$(AVAX_ID_RPC=1 AVAX_SOURCE_CHAIN=P $(cmd) -@X1 -@X2 -@X3 -l 1024 \
...
function test_avm__get_utxos_2a {
    check "$(AVAX_ID_RPC=1 \
        ...
        --start-index-address=SI_X0 --start-index-utxo=SI_UTXO -s P)" BC_ID ;
}
