...
function cmd {
    printf "./avalanche-cli.sh avax import" ;
}
...
function test_avax__import_1a {
    check "$(AVAX_ID_RPC=1 $(cmd) \
        -@ TO -s X -u USERNAME -p PASSWORD)" ;
}

function test_avax__import_1b {
    check "$(AVAX_ID_RPC=1 AVAX_TO=TO $(cmd) \
        -s X -u USERNAME -p PASSWORD)" ;
}

function test_avax__import_1c {
    check "$(AVAX_ID_RPC=1 AVAX_SOURCE_CHAIN=X $(cmd) \
        -@ TO -u USERNAME -p PASSWORD)" ;
}

function test_avax__import_1d {
    check "$(AVAX_ID_RPC=1 AVAX_USERNAME=USERNAME $(cmd) \
        -@ TO -s X -p PASSWORD)" ;
}

function test_avax__import_1e {
