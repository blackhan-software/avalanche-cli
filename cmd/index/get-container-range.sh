...
function cli {
    while getopts ":hSVYN:i:f:e:b:n:-:" OPT "$@"
    do
        if [ "$OPT" = "-" ] ; then
            OPT="${OPTARG%%=*}" ;
            OPTARG="${OPTARG#$OPT}" ;
        ...
        case "${OPT}" in
            list-options)
                cli_options && exit 0 ;;
            i|start-index)
                AVAX_START_INDEX="${OPTARG}" ;;
            f|num-to-fetch)
                AVAX_NUM_TO_FETCH="${OPTARG}" ;;
            e|encoding)
    ...
    if [ -z "$AVAX_START_INDEX" ] ; then
        cli_help && exit 1 ;
    fi
    if [ -z "$AVAX_NUM_TO_FETCH" ] ; then
        cli_help && exit 1 ;
    fi
...
function rpc_method {
    printf "index.getContainerRange" ;
}

function rpc_params {
    printf '{' ;
    printf '"startIndex":%s,' "$AVAX_START_INDEX" ;
    printf '"numToFetch":%s,' "$AVAX_NUM_TO_FETCH" ;
