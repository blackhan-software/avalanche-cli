...
function cli_help {
    ...
    usage+=" [-e|--end-time=\${AVAX_END_TIME}]" ;
    usage+=" [-#|--stake-amount=\${AVAX_STAKE_AMOUNT}[E|P|T|G|M|K]]" ;
    usage+=" [-r|--delegation-fee-rate=\${AVAX_DELEGATION_FEE_RATE}[%]]" ;
...
function cli {
    ...
    while getopts ":hSVYN:i:b:e:#:r:@:f:c:u:p:-:" OPT "$@"
    do
        if [ "$OPT" = "-" ] ; then
            OPT="${OPTARG%%=*}" ;
            OPTARG="${OPTARG#$OPT}" ;
        ...
        case "${OPT}" in
           ...
           \#|stake-amount)
            ...
            r|delegation-fee-rate)
                AVAX_DELEGATION_FEE_RATE="${OPTARG}" ;;
            @|reward-address)
    ...
    if [ -z "$AVAX_DELEGATION_FEE_RATE" ] ; then
        cli_help && exit 1 ;
    fi
...
function rpc_params {
    ...
    printf '"delegationFeeRate":%s,' "$(pct "$AVAX_DELEGATION_FEE_RATE")" ;
