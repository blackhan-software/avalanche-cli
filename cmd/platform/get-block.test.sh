...
function cmd {
    printf "./avalanche-cli.sh platform get-block" ;
}
...
function check {
    ...
    expect_d+='"id":1,' ;
    expect_d+='"method":"platform.getBlock",' ;
    expect_d+='"params":{' ;
...
function test_platform__get_block_1a {
    check "$(AVAX_ID_RPC=1 $(cmd) -b BLOCK_ID -e hex)" ;
}

function test_platform__get_block_1b {
    check "$(AVAX_ID_RPC=1 AVAX_BLOCK_ID=BLOCK_ID $(cmd) -e hex)" ;
}

function test_platform__get_block_1c {
