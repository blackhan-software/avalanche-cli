...
function cli_help {
    ...
    usage+=" [-a|--alias=\${AVAX_ALIAS}]" ;
    usage+=" [-e|--endpoint=\${AVAX_ENDPOINT}]" ;
...
function cli {
    ...
    if [ -z "$AVAX_ALIAS" ] ; then
        cli_help && exit 1 ;
    fi
    if [ -z "$AVAX_ENDPOINT" ] ; then
        cli_help && exit 1 ;
    fi
    if [ -z "$AVAX_NODE" ] ; then
...
function rpc_params {
    printf '{' ;
    printf '"alias":"%s",' "$AVAX_ALIAS" ;
    printf '"endpoint":"%s"' "$AVAX_ENDPOINT" ;
    printf '}' ;
}

###############################################################################

cli "$@" && rpc_post "$AVAX_NODE/ext/admin" "$(rpc_data)" ;
