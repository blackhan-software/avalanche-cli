...
## Auth API:
CLI_TEXT+=( "auth|When you run a node, you can require that API calls have an authorization token attached. This API manages the creation and revocation of authorization tokens. See: ${LK}https://docs.avax.network/build/avalanchego-apis/auth-api${NN}" ) ;
...
## Metrics API:
CLI_TEXT+=( "metrics|The API allows clients to get statistics about a node's health and performance. See: ${LK}https://docs.avax.network/build/avalanchego-apis/metrics-api${NN}" ) ;
CLI_HELP+=( "metrics|get-prometheus|Get Prometheus compatible metrics." ) ;

## Platform Chain (P-Chain) API:
CLI_TEXT+=( "platform|This API allows clients to interact with the P-Chain (Platform Chain), which maintains Avalanche's validator set and handles blockchain creation. See: ${LK}https://docs.avax.network/build/avalanchego-apis/platform-chain-p-chain-api${NN}")
