...
## Auth API:
CLI_TEXT+=( "auth|When you run a node, you can require that API calls have an authorization token attached. This API manages the creation and revocation of authorization tokens. See: ${LK}https://docs.avax.network/apis/avalanchego/apis/auth${NN}" ) ;
...
CLI_TEXT+=( "avm|The X-Chain, Avalanche's native platform for creating and trading assets, is an instance of the Avalanche Virtual Machine (AVM). This API allows clients to create and trade assets on the X-Chain and other instances of the AVM. See: ${LK}https://docs.avax.network/apis/avalanchego/apis/x-chain${NN}")
...
## Health API:
CLI_TEXT+=( "health|This API can be used for measuring node health. See: ${LK}https://docs.avax.network/apis/avalanchego/apis/health${NN}" ) ;
...
## Info API:
CLI_TEXT+=( "info|This API can be used to access basic information about the node. See: ${LK}https://docs.avax.network/apis/avalanchego/apis/info${NN}" ) ;
...
## IPC API:
CLI_TEXT+=( "ipcs|The IPC API allows users to create UNIX domain sockets for blockchains to publish to. When the blockchain accepts a vertex/block it will publish it to a socket and the decisions contained inside will be published to another. See: ${LK}https://docs.avax.network/apis/avalanchego/apis/ipc${NN}" ) ;
...
## Metrics API:
CLI_TEXT+=( "metrics|The API allows clients to get statistics about a node's health and performance. See: ${LK}https://docs.avax.network/apis/avalanchego/apis/metrics${NN}" ) ;
CLI_HELP+=( "metrics|get-prometheus|Get Prometheus compatible metrics." ) ;

## Platform Chain (P-Chain) API:
CLI_TEXT+=( "platform|This API allows clients to interact with the P-Chain (Platform Chain), which maintains Avalanche's validator set and handles blockchain creation. See: ${LK}https://docs.avax.network/apis/avalanchego/apis/p-chain${NN}")
