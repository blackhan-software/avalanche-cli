...
function cli_help {
    ...
    if [[ -z "$1" || "$1" == "avm" ]] ; then
        printf "\n" ;
        printf "\n${BB}Exchange Chain (X-Chain) API:${NB}\n\n$(text_for 'avm')" ;
        printf "\n" ;
    ...
    if [[ -z "$1" || "$1" == "avax" ]] ; then
        printf "\n" ;
        printf "\n${BB}Contract Chain (C-Chain) AVAX API:${NB}" ;
        printf "\n" ;
        printf "$(line_for 'avax' "$2")" ;
    fi
    ...
    if [[ -z "$1" || "$1" == "platform" ]] ; then
        printf "\n" ;
        printf "\n${BB}Platform Chain (P-Chain) API:${NB}\n\n$(text_for 'platform')" ;
        printf "\n" ;
...
## Admin API:
CLI_TEXT+=( "admin|This API can be used for measuring node health and debugging. Note that the Admin API is disabled by default for security reasons. To run a node with the Admin API enabled, use command line argument ${BB}--api-admin-enabled=true${NB}. See: ${LK}https://docs.avax.network/reference/avalanchego/admin-api${NN}" ) ;
...
## Auth API:
CLI_TEXT+=( "auth|When you run a node, you can require that API calls have an authorization token attached. This API manages the creation and revocation of authorization tokens. See: ${LK}https://docs.avax.network/reference/avalanchego/auth-api${NN}" ) ;
...
CLI_TEXT+=( "avm|The X-Chain, Avalanche's native platform for creating and trading assets, is an instance of the Avalanche Virtual Machine (AVM). This API allows clients to create and trade assets on the X-Chain and other instances of the AVM. See: ${LK}https://docs.avax.network/reference/avalanchego/x-chain/api${NN}")
...
CLI_HELP+=( "avm|create-nft-asset|Create a new non-fungible asset. No units of the asset exist at initialization. Minters can mint units of this asset using 'avm mint-nft'." ) ;
...
CLI_HELP+=( "avm|export|Send a non-AVAX from the X-Chain to the P-Chain or C-Chain. After calling this method, you must call 'avax.import' on the C-Chain to complete the transfer." ) ;
...
CLI_HELP+=( "avm|get-block|Returns the block with the given id." ) ;
CLI_HELP+=( "avm|get-block-by-height|Returns block at the given height." ) ;
CLI_HELP+=( "avm|get-height|Returns the height of the last accepted block." ) ;
...
CLI_HELP+=( "avm|issue-tx|Send a signed transaction to the network. 'encoding' specifies the format of the signed transaction. Defaults to 'hex'." ) ;
...
## Contract Chain (C-Chain) AVAX API:
CLI_HELP+=( "avax|export|Export an asset from the C-Chain to the X-Chain. After calling this method, you must call 'import' on the X-Chain to complete the transfer." ) ;
CLI_HELP+=( "avax|export-key|Get the private key that controls a given address. The returned private key can be added to a user with 'avax import-key'." ) ;
...
CLI_HELP+=( "info|acps|Get supported and objected ACPs." ) ;
...
## IPC API:
CLI_TEXT+=( "ipcs|The IPC API allows users to create UNIX domain sockets for blockchains to publish to. When the blockchain accepts a vertex/block it will publish it to a socket and the decisions contained inside will be published to another. See: ${LK}https://docs.avax.network/reference/avalanchego/ipc-api${NN}" ) ;
...
CLI_HELP+=( "platform|get-block|Returns the block with the given id." ) ;
